...
class greedy_search(object):
    ...
    def __init__(self, problem):
        ...
        from ``current`` state.
        TestObjective(current,target): function that evaluates if ``current``
        state corresponds to the ``target`` state
        '''
            ...
            for next_item in new_solutions:
                ...
                if next_item not in came_from:
                    ...
                    priority = self.problem.Heuristic(target, next_item) + \
                               self.problem.problem[current][next_item]['weight']                  
                    print(next_item + ": " + str(priority))
